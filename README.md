...
- 报名候补与退局补位：报名有空位立即占座，满员后进入候补并显示前面等待人数；玩家退局时按候补顺序自动补位，同一玩家在同一场次仅保留一条报名，重复提交会提示已报名。总览页可直接办理报名/退局，并实时展示座位、候补人数与最近一次补位结果。
...
| 后端 | Spring Boot + Java |
| 数据库 | PostgreSQL（本地开发默认使用 H2 内存库，无需安装数据库） |
| 认证 | JWT |
| 依赖 | MyBatis、Maven |
...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET | `/api/sessions` | 总览页场次列表：容量、空位、候补人数、最近补位结果 |
| POST | `/api/sessions/{id}/register` | 玩家报名，请求体 `{"playerName": "小明"}`；有空位占座，满员进候补 |
| POST | `/api/sessions/{id}/cancel` | 玩家退局；自动按候补顺序补位，每个退局最多补一人 |

规则说明：

- 同一场次按场次加锁串行处理报名/退局，数据库还有 `(场次, 玩家)` 唯一约束兜底，不会把同一空位分给两人。
- 两人同时退局时，两个事务各释放一个空位、各补队首一人，候补剩余玩家继续等待。
- 同一玩家在同一场次只保留一条报名，重复提交返回「已报名」提示。
- 数据库首次启动由 `database/init.sql` 自动建表并写入三个演示场次。
